...
test("сокращение строчными поднимается до принятого вида", () => {
  /* «ваз 2105» — это марка, а не слово: подняв только первую букву, мы
     получали «Ваз», что выглядит так же неопрятно, как исходное. */
  assert.ok(tidyListingTitle("ваз 2105 в хорошем состоянии").startsWith("ВАЗ"))
  assert.ok(tidyListingTitle("Продам камаз срочно").includes("КАМАЗ"))
  assert.ok(tidyListingTitle("авто с гбо").includes("ГБО"))
})

test("обычные слова остаются как есть", () => {
  /* Список сокращений короткий и точный: обычное слово поднимать нельзя,
     иначе середина предложения превратится в набор криков. */
  assert.equal(tidyListingTitle("Продам машину в отличном состоянии"), "Продам машину в отличном состоянии")
})
