...
test("заглушка не подменяет собой готовые серверные карточки", () => {
  /* SWR держит isLoading истинным на первом проходе даже когда
     fallbackData уже заполнил data. Проверено на продакшене: ключ и
     стартовые данные были на месте, а в разметке стояла заглушка
     «Загружаем объявления» — робот не видел ни одной карточки.

     Условие обязано учитывать наличие данных, иначе серверная отрисовка
     витрины теряет смысл. */
  assert.match(client, /isLoading && !data \?/)
})
