...
test("плашка показывается только вблизи", () => {
  /* Порог поднят с двенадцатого масштаба до шестнадцатого.

     На двенадцатом в кадре центра Уфы помещается под две сотни точек, и
     каждая рисовала плашку шириной до 290 пикселей при ячейке разведения
     в 72 — вчетверо уже плашки. Они налезали сплошным ковром: карты под
     ними не видно, цены соседних заправок сливались в кашу.

     С шестнадцатого в кадре пара кварталов и несколько точек — там плашка
     помещается и читается. */
  const page = readFileSync(new URL("../src/app/services/fuel-map/page.tsx", import.meta.url), "utf8")
  assert.match(page, /showPlate = !isCluster && zoom >= 16/)
})

test("вместо плашки метка несёт цену главной марки", () => {
  /* Без подписи карта обеднела бы: до правки человек видел цены, не
     нажимая. Одна цифра под кружком отвечает на главный вопрос «почём
     здесь», занимая место в двадцать раз меньше плашки. */
  const page = readFileSync(new URL("../src/app/services/fuel-map/page.tsx", import.meta.url), "utf8")
  assert.match(page, /markerPrice/)
  assert.match(page, /fuel-map-price/)
})
...
test("метка указывает на заправку остриём", () => {
  /* Круглая метка стояла над точкой и не говорила, к какому месту
     ...
     Остриё рисуется псевдоэлементом, а не поворотом самой кнопки:
     повёрнутая кнопка сдвигалась относительно подписи с ценой, и цена
     повисала в стороне от своей метки.
...
test("цена показывается с копейками", () => {
  ...
  /* Ровно десять копеек не теряют нуля: «64,70 ₽», а не «64,7 ₽» —
     на колонке цену пишут двумя знаками, и обрезанный хвост читается
     как другая цена. */
  assert.equal(formatFuelKopecks(6_470), "64,70")
...
test("заправка в одной точке под разными именами — это одна заправка", () => {
  /* Источники зовут одну точку по-разному: «АЗС Газпром» у одного и
     «Промышленная» у другого, «Газпром» и «Салават». По именам они не
     сходятся, и в Стерлитамаке такая заправка стояла тремя метками.

     Пятнадцать метров меньше одной колонки — на таком расстоянии разные
     АЗС не ставят, и координата надёжнее названия. Дальше пятнадцати
     метров имя снова решает: на одной территории бывают и АЗС, и
     отдельная газовая станция. */
  const route = readFileSync(new URL("../src/app/api/fuel-stations/route.ts", import.meta.url), "utf8")

  assert.match(route, /const STATION_SAME_SPOT_DISTANCE_SQUARED = \(15 \/ 111_000\) \*\* 2/)

  /* Колонка и зарядка в одной точке остаются разными: их ассортименты не
     пересекаются вовсе, и объединять их нельзя. */
  assert.match(route, /liveIsCharger === directoryIsCharger/)
})

test("отметка водителя главнее прайса источника, но не бесконечно", () => {
  /* Отметка водителя — самое ценное в сервисе: он стоит у колонки и видит
     табло. Но живёт она семь дней, а источник обновляется каждые
     пятнадцать минут, и через сутки получалось обратное: карта неделю
     держала цену, которую заправка давно сменила.

     Замер по проду: отметка сорокачасовой давности показывала АИ-95 за 67
     рублей, источник — 71,35, и человек приезжал к другой цене.

     Шесть часов — смена на заправке, за которую цена почти не меняется.
     Дальше отметка уступает источнику, но только при заметном
     расхождении: рубль на литре человек замечает, копейки нет. */
  const page = readFileSync(new URL("../src/app/services/fuel-map/page.tsx", import.meta.url), "utf8")

  assert.match(page, /const REPORT_PRIORITY_MS = 6 \* 60 \* 60 \* 1000/)
  assert.match(page, /const PRICE_DISAGREEMENT_KOPECKS = 100/)
  assert.match(page, /function pickDisplayPrice/)

  /* Совсем без источника отметка остаётся единственным, что есть у карты:
     отбрасывать её нельзя ни при каком возрасте. */
  assert.match(page, /if \(sourceKopecks === undefined\) return reported\.priceKopecks/)

  /* Человек должен видеть, чья перед ним цена: отметка соседа и прайс
...
test("у карты АЗС есть городские страницы для поиска", () => {
  ...
  assert.match(page, /Цены на бензин в \$\{where\}/)
  assert.match(page, /application\/ld\+json/)

  /* Mantine здесь нельзя: библиотека клиентская, и страница с ней пришла
     бы поисковику пустой — ровно то, ради чего страница и делалась. */
...
test("город в заголовке стоит в предложном падеже", () => {
  /* «Цены на бензин в Уфа» выдаёт машинную сборку текста: доверия такой
...
test("склонение одно на объявления и на карту", () => {
  /* Правило существовало в двух копиях: одна обслуживала заголовки
     объявлений, другая — страницы карты заправок. Копии успели
     разойтись — та, что для карты, научилась склонять области, а вторая
     нет, и объявления в Подмосковье получали «Купить в Московская
     область».

     Случаи ниже покрывают то, что умела каждая из копий: если при
     следующем слиянии что-то потеряется, тест это покажет. */
  assert.equal(cityInPrepositional("Мордовия"), "Мордовии")
  assert.equal(cityInPrepositional("Одинцово"), "Одинцово")
  assert.equal(cityInPrepositional("Химки"), "Химки")
  assert.equal(cityInPrepositional("Тверь"), "Твери")
})
...
test("сетки фильтров ужимаются ступенями, а не рвутся", () => {
  /* Сетка каталога задана жёсткими минимумами — в сумме 908 пикселей плюс
     отступы. На ноутбуке в 1024 контенту рядом с боковым меню остаётся
     около 724: панель вылезала за край, и страница получала горизонтальную
     прокрутку вместе с шапкой и подвалом.
