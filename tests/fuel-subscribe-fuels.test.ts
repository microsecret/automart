...
test("кнопки марок раскладываются по ширине экрана", () => {
  /* Марок шесть: в одну строку подписи сжимаются до нечитаемых, в
     столбик занимают полэкрана. Три в ряд — там, где помещаются; на
     узком телефоне две, иначе кнопка «Газ» ужимается до нечитаемой. */
  assert.ok(button.includes("<SimpleGrid cols={{ base: 2, xs: 3 }}"))
  assert.ok(!button.includes("<Group grow"), "остался растягивающий ряд")
})
