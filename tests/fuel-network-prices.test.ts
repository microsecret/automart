...
test("дороговизне в разумных пределах место остаётся", () => {
  // Замер по трём городам: настоящие сети укладываются в 0,90–1,09
  // медианы города. «Опти» в Москве стоит 1,09 и остаётся в сводке —
  // дороже соседей, но это рынок, а не вымысел.
  const rows = buildNetworkPrices([
    ...samples("Лукойл", Array(40).fill(7_194)),
    ...samples("Роснефть", Array(40).fill(7_194)),
    ...samples("Опти", Array(20).fill(7_830)),
  ])
  assert.equal(rows.length, 3)
  assert.equal(rows[rows.length - 1].label, "Опти")
})
