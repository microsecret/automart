...
test("пост уходит одним сообщением, а не двумя", () => {
  /* Раньше уходил альбом, а под ним отдельная строка «Открыть
     объявление:» с кнопками — Telegram не даёт прикрепить их к альбому.
     В чате это читалось как два поста подряд, причём второй с
     предпросмотром той же ссылки, на которую ведёт кнопка. */
  assert.doesNotMatch(sender, /sendMediaGroup/)
  assert.doesNotMatch(sender, /reply_to_message_id/)
})

test("кнопки стоят под самой фотографией", () => {
  // Одна фотография кнопки принимает — в этом и смысл правки.
  assert.match(sender, /sendPhoto[\s\S]{0,400}reply_markup: keyboard/)
})

test("в чат идёт первый снимок, остальные ждут на странице", () => {
  /* Показать десять фотографий в чате важно меньше, чем дать нажать
     кнопку под той, которую уже увидели. */
  assert.match(sender, /const cover = post\.photos\[0\]/)
})
