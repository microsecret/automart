...
test("production-деплой сохраняет включённое расписание из .env", () => {
  assert.match(deploy, /node --env-file=\.env -p/)
  assert.match(deploy, /AUTOMART_AUCTION_TELEGRAM_CRON="\$auction_telegram_cron_flag"/)
})
