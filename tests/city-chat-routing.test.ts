...
test("все названия из правил совпадают с чатами сети", () => {
  ...
  /* И обратно: подключённый к боту чат без городов стоит пустым, а
     объявления из его региона уходят в общий чат страны. Так пропала
     Пермь — проверка была односторонней и этого не ловила. */
  const routed = listRoutedChatTitles()
  for (const title of real) {
    if (title === "Авторынок России") continue
    assert.ok(routed.includes(title), `чат подключён, но города к нему не ведут: ${title}`)
  }
})

test("города Пермского края ведут в пермский чат", () => {
  assert.equal(pickChatTitleForCity("Пермь"), "Авторынок Пермь")
  assert.equal(pickChatTitleForCity("Березники"), "Авторынок Пермь")
  assert.equal(pickChatTitleForCity("г. Соликамск"), "Авторынок Пермь")
})
