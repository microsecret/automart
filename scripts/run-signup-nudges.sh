...
# Напоминания тем, кто открыл бота, но так и не завёл аккаунт.
#
# Кого и когда звать, решает приложение: первое напоминание через две недели
# после знакомства, дальше тем же шагом, максимум четыре. Здесь только
# регулярный вызов.
cd "$(dirname "$0")/.."
set -a
...
curl --fail --silent --show-error --connect-timeout 10 --max-time 120 \
  ...
  "${BASE_URL}/api/telegram/signup-nudges"
echo
