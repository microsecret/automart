...
const nextConfig = {
  ...
  async redirects() {
    /* Угадываемые адреса, которых в проекте нет.
     *
     * Разделы объявлений живут по `/listings/vehicle` и `/listings/part`, а
     * сам `/listings` отдавал 404: страницы с таким путём никогда не было.
     * Внутренних ссылок на него нет, но адрес очевидный — его набирают
     * руками и он встречается во внешних ссылках. Упереться в «страница не
     * найдена» на корне собственного раздела не должен никто.
     *
     * Перенаправление постоянное: адрес не появится позже, и поисковикам
     * стоит сразу запомнить каталог вместо него. */
    return [
      { source: "/listings", destination: "/category/cars", permanent: true },
    ]
  },
  async headers() {
