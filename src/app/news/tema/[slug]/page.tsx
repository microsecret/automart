 ...
 */
/**
 * Страницы тем собираются при первом обращении, а не заранее.
 *
 * Раньше здесь возвращался полный список тегов, и сборка готовила
 * страницу под каждый — десятки запросов к базе подряд. 19 сентября
 * 2026 это уронило боевой сайт: SQLite не ответила за отведённое
 * время («Operations timed out»), сборка прервалась на теге «dtp», и
 * служба перезапустилась на пустой папке — страница отдавала 502.
 *
 * Причина внешняя: на сервере работает не только эта площадка, и под
 * общей нагрузкой единственное соединение с базой отвечает медленнее.
 * Пересборка прошла с первого раза — то есть код был исправен, а
 * сборка оказалась хрупкой.
 *
 * Пустой список означает: страницы создаются по запросу и кэшируются.
 * Первый посетитель редкого тега ждёт лишние полсекунды, зато сборка
 * больше не зависит от того, свободна ли база в эту минуту.
 */
