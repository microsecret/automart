...
async function sendEmailRequest(chatId: string) {
  await telegramApi("sendMessage", {
    ...
    text: [
      ...
      "",
      /* Зачем почта — главный вопрос на этом шаге: человек уже в
         Telegram, его опознали, и просьба выглядит лишней. Из ста
         шестидесяти двух открывших бота двадцать четыре бросают именно
         здесь, получив по три напоминания. */
      "Она нужна, чтобы вы могли войти с компьютера и вернуть доступ, если потеряете Telegram. Рассылок не будет.",
      "",
  ...
  if (message.text?.trim().toLowerCase().match(/^\/(fuel|benzin|бензин)/) && message.chat.type === "private") {
    await telegramApi("sendMessage", {
      ...
      text: [
        ...
        "",
        /* Сказано ровно то, что бот умеет.

           Раньше обещалось «спрошу, какое там топливо и почём», но цену
           бот записать не может: он пишет только наличие, а форма цены
           живёт на карте. Обещание, которого не выполняют, обходится
           дороже несделанного — человек ждёт вопроса про цену, не
           получает его и перестаёт верить остальному. */
        "Дальше ничего делать не нужно. Когда вы постоите у заправки — я спрошу, какое там топливо есть. Одно нажатие, и другие водители не поедут туда впустую.",
        "",
        "Цену можно отметить на карте: там же видно, у кого дешевле.",
        "",
