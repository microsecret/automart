...
export default function HistoryCheckPage() {
  ...
  return (
    <Box p={{ base: "sm", md: "md" }} maw={720} mx="auto">
      <Stack gap="md">
        <Group gap="sm" align="center"><ThemeIcon variant="light" color="green" size={44} radius="md"><IconHistory size={22} /></ThemeIcon><Stack gap={0}><Text component="h1" c="var(--market-ink)" ff="var(--font-display),sans-serif">Проверка истории</Text><Text size="xs" c="var(--market-muted)">Заявка на проверку собственных автомобилей с прозрачным статусом</Text></Stack></Group>
        ...
        {status === "unauthenticated" ? (
          ...
          <Paper withBorder radius="md" p="xl"><Group justify="center"><Loader color="green" size="sm" /><Text size="sm" c="dimmed">Загружаем ваши автомобили…</Text></Group></Paper>
        ) : error ? (
        ...
        {request && <Paper withBorder radius="md" p="lg" style={{ borderColor: "var(--market-success-line)", background: "var(--market-success-surface)" }}><Stack gap="md"><Group gap="sm"><ThemeIcon size={38} radius="xl" color="green" variant="light"><IconCheck size={20} /></ThemeIcon><Stack gap={0}><Text fw={700} c="var(--market-success-text)">Заявка сохранена</Text><Text size="xs" c="var(--market-success-text)">Статус: ожидает подключения проверенного источника</Text></Stack></Group><Timeline bulletSize={22} lineWidth={2} color="green"><Timeline.Item bullet={<IconCheck size={13} />} title="Заявка создана"><Text size="xs" c="dimmed">{new Date(request.request.createdAt).toLocaleString("ru-RU")}</Text></Timeline.Item><Timeline.Item bullet={<IconClock size={13} />} title="Источник данных не подключён"><Text size="xs" c="dimmed">Мы не подменяем этот этап случайным результатом.</Text></Timeline.Item></Timeline><Text size="xs" c="dimmed">Номер заявки: {request.request.id}</Text></Stack></Paper>}
      </Stack>
    </Box>
  )
}
