        ...
        {status === "unauthenticated" ? (
          ...
          <Paper withBorder radius="md" p="lg">
            <Stack gap="md">
              ...
              <Button onClick={calculate} loading={submitting} disabled={!vehicleId} color="indigo" size="md" leftSection={<IconCalculator size={18} />}>Рассчитать ориентир</Button>
            </Stack>
          </Paper>
        )}
          ...
          <Stack gap="md">
            <Paper radius="md" p="xl" withBorder style={{ background: "linear-gradient(135deg, #eef2fb 0%, #fff 100%)", borderColor: "#b9caee" }}>
              <Stack gap="sm" align="center">
                ...
                <Text fz="2.2rem" fw={800} c="var(--market-accent-text)" ff="var(--font-display),sans-serif" lh={1}>{formatPrice(result.estimatedValue)}</Text>
                <Group gap="xl">
                  <Stack gap={0} align="center"><Group gap={4}><IconTrendingDown size={14} color="#e11d48" /><Text size="xs" c="var(--market-muted)">Нижняя граница</Text></Group><Text fw={700} fz="md" c="var(--market-danger-text)">{formatPrice(result.min)}</Text></Stack>
                  <Stack gap={0} align="center"><Group gap={4}><IconTrendingUp size={14} color="#059669" /><Text size="xs" c="var(--market-muted)">Верхняя граница</Text></Group><Text fw={700} fz="md" c="var(--market-success-text)">{formatPrice(result.max)}</Text></Stack>
                </Group>
              </Stack>
            </Paper>
