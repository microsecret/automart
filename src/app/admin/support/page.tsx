      ...
      <Stack gap="md">
        <Card className="admin-workspace__hero" radius="md" p={{ base: "md", sm: "lg" }}>
          <Group justify="space-between" align="center" gap="md" wrap="wrap">
            <Group gap="sm" wrap="nowrap">
              ...
              <Stack gap={2}>
                <Title order={1} size="h2" c="white">Центр поддержки</Title>
        ...
        <Grid gutter="md" align="stretch">
          ...
          <Grid.Col span={{ base: 12, lg: 8 }}>
            {/* Высота фиксировалась в 640 пикселей и на телефоне тоже.

                На экране в 568 пикселей такой блок не помещался, а форма
                ответа лежит у его нижнего края: чтобы до неё добраться,
                надо было прокрутить страницу, но при фокусе в поле
                клавиатура занимает почти половину экрана и кнопка
                «Отправить» уходила под неё. Ответить клиенту с телефона
                было нельзя — при том что это ежедневная работа.

                На телефоне высота задаётся минимумом, а не жёстко: блок
                растёт под содержимое и прокручивается вместе со
                страницей. На широком экране остаётся как было — там две
                колонки, и фиксированная высота держит их вровень. */}
            <Paper withBorder radius="md" mih={{ base: 420 }} h={{ lg: 720 }} p="md">
              {!selectedId ? (
                ...
                <Stack h="100%" gap="sm">
                  ...
                  {ticket.status === "CLOSED" ? (
                    ...
                    /* Кнопка уходит под поле, а не жмётся рядом: с запретом
                       переноса поле ответа и «Отправить» делили одну строку,
                       и на узком экране полю оставалось около ста сорока
                       пикселей — писать ответ клиенту в такое поле нельзя. */
