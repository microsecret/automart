    ...
    <Paper radius="lg" p="md" withBorder className="admin-auction-inquiry-row">
      <Group gap="md" align="flex-start" wrap="wrap">
        ...
        <Stack gap={6} style={{ flex: "1 1 380px", minWidth: 0 }}>
          ...
          <Group gap="xs" wrap="wrap"><Text size="xs" c="dimmed"><IconClock size={12} style={{ verticalAlign: -2 }} /> {formatRelativeDate(inquiry.createdAt)}</Text><Text size="xs" c="dimmed">· {formatRub(listing?.finalPrice)}</Text>{inquiry.assignedPartner && <Text size="xs" c="indigo.7" fw={700}>· Партнёр: {inquiry.assignedPartner.name || "назначен"}</Text>}</Group>
        </Stack>
