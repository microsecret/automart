...
export default function DeliveryOrderPage() {
  ...
  return <Box p={{ base: "sm", md: "lg" }}>
    <Stack gap="md">
      ...
      <Paper radius="lg" p={{ base: "md", md: "xl" }} style={{ color: "white", background: "linear-gradient(122deg, #0a1733, #1f3d7a 62%, #5042bd)", overflow: "hidden" }}>
      ...
      <SimpleGrid cols={{ base: 1, xl: 3 }} spacing="md">
        <Stack gap="md" style={{ gridColumn: "span 2" }}>
          <Paper withBorder radius="lg" p="md"><Group justify="space-between" align="flex-start" wrap="wrap"><Stack gap={2}><Group gap="xs"><ThemeIcon color="indigo" variant="light" radius="md"><IconClock size={17} /></ThemeIcon><Text fw={800}>Следующий шаг</Text></Group><Text c="dimmed" size="sm">{order.nextAction || statusMeta.description}</Text>{order.nextActionAt && <Text size="xs" c="indigo">Ориентир: {formatDate(order.nextActionAt)}</Text>}</Stack>{permissions.canManage && <Button size="xs" color="indigo" leftSection={<IconPlus size={14} />} onClick={() => setEventOpened(true)}>Подтвердить этап</Button>}</Group></Paper>
