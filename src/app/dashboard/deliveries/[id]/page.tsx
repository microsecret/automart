  ...
  return <Box p={{ base: "sm", md: "lg" }}>
    <Stack gap="md">
      ...
      <Paper radius="md" p={{ base: "md", md: "xl" }} style={{ color: "white", background: "linear-gradient(122deg, #0a1733, #1f3d7a 62%, #5042bd)", overflow: "hidden" }}>
        <Stack gap="md"><Group justify="space-between" align="flex-start" wrap="wrap"><Stack gap={6}><Group gap="xs"><Badge variant="white" color={statusMeta.color}>{statusMeta.shortLabel}</Badge><Text size="xs" c="rgba(255,255,255,.62)">{order.kind === "PART" ? "Запчасть под заказ" : "Международная доставка транспорта"}</Text></Group><Title order={1} fz={{ base: "22px", md: "34px" }} ff="var(--font-display), sans-serif">{order.title}</Title><Text size="sm" c="rgba(255,255,255,.72)">{order.originCountry} {order.originCity ? `· ${order.originCity}` : ""} <IconArrowRight size={14} style={{ verticalAlign: "middle" }} /> {order.destinationCity}{order.destinationRegion ? `, ${order.destinationRegion}` : ""}</Text></Stack><ThemeIcon size={50} radius="lg" variant="white" color="indigo"><IconTruckDelivery size={27} /></ThemeIcon></Group>
        ...
        <Stack gap="md" style={{ gridColumn: "span 2" }}>
          ...
          <Paper withBorder radius="md" p="md"><Group justify="space-between" mb="md"><Group gap="xs"><ThemeIcon color="indigo" variant="light" radius="md"><IconRoute size={17} /></ThemeIcon><Text fw={800}>Лента маршрута</Text></Group><Badge variant="light" color={statusMeta.color}>{statusMeta.shortLabel}</Badge></Group>
            <Timeline active={Math.max(0, order.events.length - 1)} bulletSize={26} lineWidth={2}>
              {order.events.map((item) => { const meta = DELIVERY_STATUS_META[item.status as keyof typeof DELIVERY_STATUS_META] || statusMeta; return <Timeline.Item key={item.id} bullet={<IconCheck size={14} />} color={meta.color} title={<Group gap={6}><Text size="sm" fw={700}>{item.title}</Text>{item.source !== "MANUAL" && <Badge size="xs" variant="light" color="gray">{sourceLabel(item.source)}</Badge>}</Group>}><Stack gap={4} mt={4}><Text size="xs" c="dimmed">{item.description || meta.description}</Text><Text size="xs" c="var(--market-muted)">{formatDateTime(item.completedAt)} · {item.author?.name || "Система"}</Text>{item.expectedAt && <Text size="xs" c="indigo">Ожидаемый срок: {formatDate(item.expectedAt)}</Text>}</Stack></Timeline.Item> })}
            </Timeline>
          </Paper>
