...
function AuctionDetail() {
  ...
  /* К повтору добавлен выход в каталог.

     Замер страницы несуществующего лота: показывалось «Лот недоступен»
     с единственной кнопкой «Повторить» — человеку предлагают повторить
     загрузку того, чего нет. Если лот снят или продан, повтор не
     поможет ни на десятой попытке, а других лотов на площадке тысячи. */
  if (error) return <Container py={80}><AsyncErrorState title="Лот недоступен" description="Возможно, он уже продан или снят с площадки. В каталоге тысячи других лотов — они обновляются каждый час." onRetry={() => void mutate()} backHref="/auctions" backLabel="Вернуться к лотам" /></Container>
