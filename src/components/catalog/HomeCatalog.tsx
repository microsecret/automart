...
export default function HomePage(p: HomePageProps = {}) {
  ...
  useEffect(() => {
    ...
    /* Остальные фильтры панели. Раньше в адресе жили только девять из
       тридцати: человек настраивал двенадцать условий, обновлял страницу
       — и терял всё, а ссылку на выборку отправить было нельзя. */
    setMileageFrom(params.get("mileageFrom") || "")
    setMileageTo(params.get("mileageTo") || "")
    setKeywords(params.get("keywords") || "")
    setTransmission(params.get("transmission") || null)
    setFuelType(params.get("fuelType")?.split(",").filter(Boolean) || [])
    setDriveType(params.get("driveType") || null)
    setBodyType(params.get("bodyType")?.split(",").filter(Boolean) || [])
    setSubtype(params.get("subtype")?.split(",").filter(Boolean) || [])
    setEngineVolumeFrom(params.get("engineVolumeFrom") || "")
    setEngineVolumeTo(params.get("engineVolumeTo") || "")
    setPowerFrom(params.get("powerFrom") || "")
    setPowerTo(params.get("powerTo") || "")
    setColor(params.get("color") || null)
    setCondition(params.get("condition")?.split(",").filter(Boolean) || [])
    setSteeringWheel(params.get("steeringWheel") || null)
    setDocumentsStatus(params.get("documentsStatus") || null)
    setDamageInfo(params.get("damageInfo") || null)
    setSellerType(params.get("sellerType") || null)
    setAvailability(params.get("availability") || null)
    setCustomsCleared(params.get("customsCleared") === "true" ? true : params.get("customsCleared") === "false" ? false : null)
    setOwnersCountFrom(params.get("ownersCountFrom") || "")
    setOwnersCountTo(params.get("ownersCountTo") || "")
  ...
  useEffect(() => {
    ...
    if (driveType) next.set("driveType", driveType)
    if (bodyType.length) next.set("bodyType", bodyType.join(","))
    if (subtype.length) next.set("subtype", subtype.join(","))
    if (engineVolumeFrom) next.set("engineVolumeFrom", engineVolumeFrom)
    if (engineVolumeTo) next.set("engineVolumeTo", engineVolumeTo)
    if (powerFrom) next.set("powerFrom", powerFrom)
    if (powerTo) next.set("powerTo", powerTo)
    if (color) next.set("color", color)
    if (condition.length) next.set("condition", condition.join(","))
    if (steeringWheel) next.set("steeringWheel", steeringWheel)
    if (documentsStatus) next.set("documentsStatus", documentsStatus)
    if (damageInfo) next.set("damageInfo", damageInfo)
    if (sellerType) next.set("sellerType", sellerType)
    if (availability) next.set("availability", availability)
    if (customsCleared !== null) next.set("customsCleared", String(customsCleared))
    if (ownersCountFrom) next.set("ownersCountFrom", ownersCountFrom)
    if (ownersCountTo) next.set("ownersCountTo", ownersCountTo)
...
/* Пришёл по ссылке с настроенным поиском — панель строится сразу.
  ...
  /* Первый экран показывается всем — как в макете площадки.

     Прежде вошедшему его не показывали, и рассуждение было верным для
     тогдашнего героя: полноэкранная витрина с обещаниями «от первого
     поиска до сделки» обращена к тому, кто решает, оставаться ли на
     площадке.

     Сегодняшний герой — не реклама. В нём поиск и пять действий:
     купить, продать, аукционы, запчасти, заправки. Это рабочие
     инструменты, и вошедшему они нужны не меньше. Без них он видел
     голое поле поиска на пустом фоне — сайт выглядел незаконченным
  ...
  return (
    <Box p={{base:"sm",md:"md"}}><Stack gap="var(--section-gap)" className="home-page-stack">
          ...
          <Box className="home-auctions__content">
            <Group justify="space-between" align="flex-start" wrap="wrap" gap="lg">
              <Box maw={650}>
                ...
                {/* Заголовок отвечает «что я здесь получу», а не описывает
                    жанр сайта: «Найдите свой маршрут…» не говорил ничего.
                    Цифра живая — та же статистика, что питает плашку шагов. */}
                {/* Кегль, вес, гарнитура и интерлиньяж заданы в globals.css по
                    атрибуту data-lw-hero, а не здесь.

                    Раньше тут стояли fw={800}, fz={{base:28,md:42}} и
                    ff="var(--font-display)". Встроенный стиль сильнее любого
                    правила из файла стилей, поэтому попытка сменить шрифт
                    заголовков в CSS не доходила до страницы: замер живой
                    вёрстки показывал Montserrat 800 при том, что правило
                    просило антикву. В разметке остаётся только цвет — он
                    зависит от подложки, а не от типографики. */}
                <Text component="h1" data-lw-hero c="white">
                  Машины с мировых аукционов и от владельцев
                </Text>
                ...
                <Group gap="xs" mt={24} wrap="nowrap" className="home-hero__search">
                  <TextInput
                    ...
                    /* Короткая подсказка.
                       ...
                       На телефоне полю остаётся около 210 пикселей: 46
                       съедает значок лупы, ещё четверть ширины — кнопка
                       «Найти». Прежнее «Марка, модель или ключевое слово»
                       обрывалось на «клю…» — оборванная подсказка не
                       объясняет, что искать, а выглядит поломкой.

                       Короткая форма понятна и на широком экране: слово
                       «ключевое» ничего не добавляло, искать по нему всё
                       равно можно. */
      ...
      {/* Здесь стояло отдельное поле поиска для вошедших.

          Оно появилось, когда первый экран им не показывали: без него
          человек, уже вошедший на площадку, открывал главную и видел
          пустоту. Но выглядело это именно так — голое поле на пустом
          фоне, будто страница не догрузилась.
      ...
      {/* Направления, объяснение и аукционы стоят ниже каталога.

          Замер главной на телефоне: герой, потом направления, потом
          «Что здесь можно сделать», потом аукционы — и только на 2173
          пикселе первая машина. Почти три экрана прокрутки мимо
          объяснений для того, кто пришёл смотреть объявления.

          Теперь сразу после героя идёт каталог, а эти три блока — под
          ним: они отвечают на вопрос «что тут ещё есть», который
          возникает уже после просмотра товара. */}
      ...
      <Group id="catalog" justify="space-between" align="center" className="catalog-heading">
        ...
        {/* Сортировка прячется, когда сортировать нечего.

            Замер поиска по бессмыслице: ноль объявлений, а рядом стоят
            «Дешевле», «Дороже» и подсвеченная синим «Новые» — будто
            список отсортирован. Человек видит работающие кнопки над
            пустотой и не понимает, сломался ли поиск.

            Условие на data: пока данные не пришли, строка остаётся —
            иначе она мигала бы при каждой загрузке. */}
        <Group gap="xs" wrap="nowrap" display={data && (data.pagination?.total ?? 0) === 0 ? "none" : undefined}>
