...
/* Миниатюра лота — через тот же кэш фото, что у публичного каталога:
   прямые ссылки площадок закрыты для вставки на чужих сайтах, и список
   в админке показывал пустые квадраты. */
import { auctionThumbnailImageUrl } from "@/lib/media-url"
...
export default function AuctionLotAdministration() {
  ...
  return <Paper withBorder radius="md" p="md">
    <Stack gap="sm">
      ...
      {isLoading ? <Text size="sm" c="dimmed">Загружаем лоты…</Text> : error ? <Text size="sm" c="var(--market-danger-text)">Не удалось загрузить лоты.</Text> : data?.lots.length ? <Stack gap={6} mah={430} style={{ overflowY: "auto" }}>{data.lots.map((lot) => <Paper key={lot.id} withBorder radius="md" p="xs"><Group justify="space-between" gap="sm" wrap="nowrap"><Group gap="sm" wrap="nowrap" miw={0}>{lot.imageUrl && <Image src={auctionThumbnailImageUrl(lot.imageUrl)} w={54} h={42} radius="sm" fit="cover" alt="" />}<Box miw={0}><Group gap={6} wrap="wrap"><Text size="sm" fw={700} lineClamp={1}>{lot.make} {lot.model} {lot.year}</Text><Badge size="xs" variant="light" color="orange">{lot.source}</Badge></Group><Text size="xs" c="dimmed">{lot.lotNumber ? `Лот ${lot.lotNumber} · ` : ""}{formatPriceShort(lot.finalPrice)}{lot.adminHiddenReason ? ` · ${lot.adminHiddenReason}` : ""}</Text></Box></Group><Group gap={6} wrap="nowrap"><Button component={Link} href={`/auctions/${lot.id}`} target="_blank" size="compact-xs" variant="subtle">Открыть</Button><Button size="compact-xs" variant="light" color={lot.adminHiddenAt ? "teal" : "red"} leftSection={lot.adminHiddenAt ? <IconEye size={13} /> : <IconEyeOff size={13} />} onClick={() => lot.adminHiddenAt ? void changeVisibility(lot, "RESTORE") : setSelected(lot)}>{lot.adminHiddenAt ? "Вернуть" : "Скрыть"}</Button></Group></Group></Paper>)}</Stack> : <Text size="sm" c="dimmed">В этом разделе лотов нет.</Text>}
    </Stack>
