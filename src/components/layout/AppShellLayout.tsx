...
export default function AppShellLayout({ children }: { children: React.ReactNode }) {
  ...
  /* Во всю ширину — только страницы одного товара.
   *
   * Раньше сюда входили главная, каталог, поиск и аукционы: меню
   * занимало 236 пикселей там, где человек смотрит товар, и казалось,
   * что оно только отодвигает карточки. Теперь у витрин три колонки —
   * меню слева, сводка справа, товар в центре, — и ширина уходит не в
   * поля по краям, а в данные.
   *
   * На карточке машины и странице лота колонки убраны намеренно: там
   * нужны фотографии и характеристики, а предложение уйти в «Мото»
   * работает против сделки.
   *
   * Запчасти отсюда тоже убраны, но по другой причине: это витрина, а
   * не карточка товара. Раньше страница шла без обеих колонок — левое
   * меню снималось здесь, правой не было вовсе, и треть экрана
   * пустовала. */
  ...
  /* Правая колонка со сводкой площадки — третья в каркасе.
   *
   * Вместе с боковым меню слева она даёт три колонки: навигация, товар,
   * сводка. Ширина уходит в данные, а не в поля по краям экрана — этим
   * сайт и отличается от Дрома с Авто.ру, где главная идёт одной
   * колонкой, а обе стороны пустуют.
   *
   * Где её нет и почему:
   ...
   *     убыток;
   *   — кабинет и админка: это рабочие столы, сводка площадки на них
   *     лишняя;
  ...
  const showAside = Boolean(
    ...
    pathname === "/news" ||
    pathname === "/forum" ||
    /* Запчасти — витрина, как каталог машин, и колонка ей полагается.
       Замер базы на 19 сентября 2026: запчастей ноль, объявлений о них
       тоже. Пока раздел пуст, колонка со свежими лотами и новостями
       рядом с пустой страницей выглядела бы насмешкой — но и убирать её
       навсегда незачем: появятся запчасти, появится и сводка.

       Оставляем: виджеты сами прячутся при нехватке данных, а лоты и
       курсы в колонке не зависят от запчастей и наполнят её честно. */
  ...
  return (
    <AppShell
      ...
      header={{ height: 68 }}
      /* 260 пикселей вместо 236: в узкой колонке подписи «Воздушный
         транспорт» и «Мировые аукционы» переносились на вторую строку, и
         строка меню вырастала вдвое. Ширина взята у площадки-образца,
         там она тоже 260–280. */
      navbar={{ width: 260, breakpoint: "md", collapsed: { mobile: !mobileOpened, desktop: isFullWidthRoute } }}
      ...
      <AppShell.Navbar className="market-app-navbar" p={0} style={{ background: "var(--market-surface-subtle)", borderRight: "1px solid var(--market-line)" }}>
        <AppShell.Section grow component={ScrollArea} type="hover" scrollbarSize={5}>
          ...
          <Stack gap={4} p="xs">
               ...
               * вещей в ряду и читается наравне с «Войти». В боковом
               * меню она стоит над всей навигацией и первой попадает в
               * поле зрения — так устроено у площадки RawMart, откуда
               * взят каркас.
               *
               * Видна только на широких экранах: на телефоне боковое
               * меню открывается поверх страницы, и там та же кнопка
               * уже стоит в нижней панели. */}
              <Button
                component={Link}
                href={CREATE_VEHICLE_HREF}
                prefetch={false}
                visibleFrom="md"
                size="sm"
                leftSection={<IconPlus size={16} stroke={2.2} />}
                className="sidebar-create-cta header-create-cta"
              >
                Подать за 2 клика
              </Button>
