          ...
          <Group gap={2} visibleFrom="md" wrap="nowrap" className="market-app-header__tabs">
            {catalogueNavigation.map((item) => (
              <Button
                ...
                color="indigo"
                size="compact-sm"
                /* Значков у пунктов меню нет — как в макете площадки.

                   Их ставили, чтобы на узком экране подпись могла
                   уступить место значку. Но правило, прятавшее подписи,
                   уже убрано (оно гасило пункты целиком), а девять
                   значков подряд в одной полосе создавали рябь: глаз
                   цеплялся за картинки вместо слов. В макете меню
                   набрано только текстом.

                   Ряд стал короче на девять значков с отступами — около
                   сотни пикселей, которых как раз не хватало. */
                title={item.label}
