...
/* Коралл — цвет действия.

   Имя `indigo` осталось прежним намеренно: Mantine раздаёт палитру по
   имени, и оно встречается в разметке под сотню раз. Переименование
   означало бы правку каждого места ради строки, которую видит только
   разработчик, — при этом любое пропущенное вхождение молча вернуло бы
   синий.

   Ступени идут от почти белой подложки к глубокому кирпичному: светлые
   работают подложкой выделения, средние — кнопкой, тёмные держат текст
   ссылки на белом. */
const indigo: MantineColorsTuple = [
  "#fff1ef", "#ffe8e5", "#ffc9c2", "#ffa094", "#ff7a6c",
  "#ff5c4b", "#ff4d3d", "#e63a2b", "#c22e21", "#8f2018",
]

/* Лайм — второй голос.

   Не дублирует коралл, а противопоставлен ему: коралл зовёт нажать, лайм
   отмечает выгоду и выделяет слово в заголовке. Текст на лайме всегда
   тёмный — светлый даёт контраст 1.4 и не читается ни на одной ступени. */
const lime: MantineColorsTuple = [
  "#f8ffe8", "#f2ffd9", "#e3fdb3", "#d2fa85", "#c4f75c",
  "#b5f23d", "#a8e02f", "#8ebd22", "#6f951a", "#4f6b12",
]
...
export const theme = createTheme({
  primaryColor: "indigo",
  // Ступень 6 — ровно #ff4d3d из макета. Белый текст на нём даёт 3.4:
  // для крупного текста кнопки этого достаточно, для мелкого нет, поэтому
  // подписи на коралловых плашках нигде не опускаются ниже 14px.
  ...
  shadows: {
    xs: "0 1px 2px rgba(15, 17, 23, 0.04)",
    sm: "0 1px 2px rgba(15, 17, 23, 0.04)",
    md: "0 4px 12px -6px rgba(15, 17, 23, 0.12)",
    lg: "0 16px 40px -16px rgba(15, 17, 23, 0.26), 0 4px 12px -6px rgba(15, 17, 23, 0.12)",
    xl: "0 24px 56px -20px rgba(15, 17, 23, 0.32)",
  },
    ...
    Button: {
      ...
      styles: {
        root: {
          ...
          // Кнопка ведёт себя как физическая клавиша: под курсором чуть
          // выходит навстречу, при нажатии уезжает в свою тень и та
          // схлопывается. Прежний подъём на размытую тень в этом языке
          // читался бы как чужая деталь: размытых теней на странице
          // больше нет.
    ...
    Card: {
      ...
      styles: {
        root: {
          ...
          // Карточку держит контур, не тень. Тень оставлена нулевой
          // намеренно: на белом фоне десяток размытых пятен превращает
          // ленту в рябь, и ни одна карточка не выходит вперёд.
          boxShadow: "none",
        },
      },
    },
