...
/* Третья ступень равна --market-line из globals.css.
   Была #cfd6e2 против #ccd5e3 у переменной: три единицы разницы, на глаз
   неразличимые, но замер главной нашёл на странице оба тона сразу — 141
   линию одного и 25 другого. Два значения для одной задачи. */
const gray: MantineColorsTuple = [
  "#fbfcfd", "#f2f4f8", "#e2e6ee", "#ccd5e3", "#8b97ad",
...
export const theme = createTheme({
  ...
  /* Расстояния пикселями, а не в rem.

     Комментарии обещали 6, 10, 16, 28 и 52 — и были бы правы при базе
     страницы в 16 пикселей. База у нас 15, поэтому rem давали 5.625,
     9.375, 15, 26.25 и 48.75. Ни одно из пяти не целое.

     Дробное расстояние само по себе не беда, но граница блока, которая
     на него опирается, попадает между пикселями экрана и рисуется
     размытой. Замер главной нашёл такие значения сотнями: gap 6.5625 у
     шапок панелей, gap 3.75 у подвала карточки, отступ 5.625 у стеков.
     Каждая линия чуть мягче, чем должна быть, и вся страница выглядит
     менее чёткой — при том что ни один блок не стоит криво.

     Радиусы в этой теме уже в пикселях, и у них такой беды нет. */
  spacing: {
    xs: "6px",
    sm: "10px",
    md: "16px",
    lg: "28px",   // между блоками внутри раздела
    xl: "52px",   // между разделами страницы
  },
