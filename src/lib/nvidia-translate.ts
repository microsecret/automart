...
/**
 * Сколько токенов запросить под ответ.
 *
 * Перевод не бывает многократно длиннее исходника, поэтому потолок считается
 * от него. Нижняя граница оставляет место коротким полям вроде цвета, верхняя
 * страхует описание целиком.
 */
function responseTokenBudget(text: string) {
  return Math.min(2_000, Math.max(120, Math.ceil(text.length * 3)))
}
