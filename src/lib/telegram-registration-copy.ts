...
export function describePendingSteps(step: TelegramRegistrationStep) {
  ...
  const headline = done > 0
    ...
    // Заголовок уведомления уже объяснил, зачем регистрироваться, поэтому
    // здесь только сами шаги — повтор про «незавершённую регистрацию» читался
    // как упрёк.
    : "Три коротких шага в личном чате с ботом:"
