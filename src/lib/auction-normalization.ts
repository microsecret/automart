...
const AUCTION_MAKE_ALIASES: Readonly<Record<string, string>> = {
  ...
  // Китайские витрины пишут марку и короткой формой, и с суффиксом «汽车»
  // («автомобили»), поэтому в словаре нужны оба варианта. Без Haval, Chery и
  // Geely в коротком написании отсеивались самые ходовые для России марки.
  "哈弗": "Haval",
  "哈佛": "Haval",
  "长城哈弗": "Haval",
  "坦克": "Tank",
...
const MODEL_PHRASE_TERMS: ReadonlyArray<readonly [RegExp, string]> = [
  ...
  // Длинные варианты идут первыми: иначе «модель» съедала начало фразы
  // «модельный год» и в названии оставался обрывок «ный год».
  [/\s*(?:года выпуска|модельного года|модельный год|года|модель)/gi, ""],
