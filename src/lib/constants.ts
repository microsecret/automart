...
const VEHICLE_IDENTITY_META: Record<TransportVehicleType, VehicleIdentityMeta> = {
  CAR: {
    ...
    /* Сказано и зачем нужен, и где взять.

       Прежнее описание объясняло только смысл, и человек, не помнящий VIN
       наизусть, вставал из-за стола искать его неизвестно где — часть из
       них не возвращалась. VIN есть в свидетельстве о регистрации и
       продублирован на кузове: обе подсказки короче похода в поисковик. */
    description: "Есть в СТС и на кузове под лобовым стеклом. Нужен для проверки истории и защиты от дублей.",
    maxLength: 17,
  },
  MOTORCYCLE: {
    field: "vin",
