...
export const TELEGRAM_MENU_NAVIGATION = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: "part-requests", label: "Заявки на запчасти", href: "/dashboard/part-requests?from=telegram" },
...
/* Порядок и подписи те же, что на телефоне.

   Наборы разошлись: на сайте заправки стояли вторыми и назывались
   «Заправки», в мини-приложении — последними и «Бензин». Человек
   открывает то одно, то другое в течение дня и каждый раз ищет кнопку
   заново; одно и то же место называлось по-разному, и это читается как
   два разных сервиса.

   Совпадает и место, и слово. Заправки вторые по той же причине, что и
   на сайте: карту открывают по нескольку раз за поездку, а аукционы
   смотрят вдумчиво и за столом — они остались в меню.

   Расхождения два, и оба вынужденные. Третье место: на сайте «Подать», а
   здесь подача переехала на главную кнопку Telegram — дублировать её
   вкладкой незачем, и место отдано аукционам, которых иначе в панели нет
   вовсе. Четвёртое: на сайте «Форум», а здесь новости — форум внутри
   приложения не написан, и вкладка увела бы на обычный сайт с десктопной
   шапкой во весь экран телефона, откуда в ленту не вернуться. Так уже
   было с «Продать» и «Форумом», и это чинили. */
export const TELEGRAM_TAB_NAVIGATION = [
  { id: "vehicles", label: "Свежее", href: "/telegram" },
  { id: "fuel", label: "Заправки", href: "/services/fuel-map?from=telegram" },
  { id: "auctions", label: "Аукционы", href: "/telegram?tab=auctions" },
  { id: "news", label: "Новости", href: "/telegram?tab=news" },
  { id: "chats", label: "Чаты", href: "/telegram?tab=chats" },
] as const satisfies readonly NavigationItem[]
