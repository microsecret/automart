...
export function getGenericIdentity(station: IdentifiableStation): NetworkIdentity {
  ...
  const namedGasOnly = !fuels && (
    /(^|\W)(агзс|агнкс|автогаз|метан|пропан)(\W|$)/.test(name)
    /* Газовые сети зовут себя «ПетролГаз», «MGaz», «Роспропан» — слово
       склеено с названием, и проверка по отдельному слову их не ловила:
       в Уфе так терялись сорок четыре точки, выходившие на карту пустой
       плашкой без единой марки.

       Защита от Газпромнефти сохраняется двумя условиями: правило
       работает только при пустом ассортименте (у Газпромнефти он
       заполнен) и исключает саму основу «газпром». */
    /* Требуем явного указания на газ, а не основу «газ» в названии.

       Первая попытка ловила всё со словом «газ» и записала в газовые
       обычный «Газпром» без ассортимента — бензиновую заправку, куда
       водитель на ГБО приехал бы зря. Слово «газ» внутри имени сети
       ничего не значит; значит только тип станции: АГЗС, АГНКС,
       газомоторное топливо, сжиженный газ, пропан, метан.

       «ПетролГаз» и «MGaz» такой проверкой не ловятся — и это честно:
       по одному слогу в названии нельзя утверждать, что там за колонка. */
    || /(агзс|агнкс|автогаз|газомотор|трансгаз|сжиженн|пропан|метан|lpg|cng)/.test(name)
  )
